...
## grblHAL-glowforge repository rules

This repository is the GRBL-mode controller: a grblHAL driver for the stock
Glowforge control board. The grblHAL core is a git submodule at `src/grbl`
that points at the project's core fork. The machine constants are in
`src/boards/glowforge.h`.

The site page is
<https://docs.forgefirm.org/technical/forgefirm/grblhal-driver/>.
...
- **The armed window is the operator's consent, and it is per job.** Nothing
  may open it except the physical button press, and nothing may widen it.
- **The dose model masks the core's fire state and is never a source of
  one.** Keep it that way: that property is what holds the model out of the
  safety argument.
...
- A change to the laser path gets a harness rule in the same commit.
